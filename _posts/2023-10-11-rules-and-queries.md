...
* `product_count` - Number of products that may be sent to the application during this rule application. The application will never receive more products than this value across all data requests of this rule application. This field is only informative and cannot be changed by the application. The value is based on the products this apply process works with - by default only products changed since the last apply, or all products when a full apply is performed (e.g. it was forced, the rules changed, or the previous apply failed) - narrowed down by the queries the rule is applied on. How precise the value is depends on those queries:
    + Rules applied on input queries only, or on no query at all, get the exact number of products the application will receive. Input values do not change during the apply, so it is already known which products the rule's queries match.
    + Rules applied on a query that searches in output values get only an upper bound. Such a query is evaluated against values produced by the rules applied before this one, so what it matches is not known yet when the settings request is sent, and the application typically receives fewer products.
    + If the number cannot be determined for an unexpected reason, the field is `null`.
